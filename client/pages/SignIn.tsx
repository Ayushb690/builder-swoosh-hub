...
export default function SignIn() {
  ...
  const login = () => {
    ...
    if (uid === expect.id && pwd === expect.password) {
      const user =
        role === "User"
          ? { name: "Patta Holder", role: "User" }
          : { name: "Authority", role: "Authority" };
      setUser(user);
          ...
          {step === 1 ? (
            <>
              ...
              <div className="mt-6 space-y-3">
                <label
                  className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer ${role === "User" ? "ring-2 ring-ring" : ""}`}
                >
                  <input
                    type="radio"
                    name="role"
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Upload and track claim status
                    </div>
                  </div>
                </label>
                <label
                  className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer ${role === "Authority" ? "ring-2 ring-ring" : ""}`}
                >
                  <input
                    type="radio"
                    name="role"
              ...
              <button
                onClick={proceed}
                className="mt-6 w-full h-10 rounded-md bg-primary text-primary-foreground"
              >
                Continue
              </button>
              <button
                onClick={() => nav("/")}
                className="mt-2 w-full h-10 rounded-md border"
              >
                Back
              </button>
            </>
            ...
            <>
              <p className="text-sm text-muted-foreground mt-1">
                Use dummy credentials shown below
              </p>
